...
class Save extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($data) {
            ...
            $model = $this->_objectManager->create(\Magendoo\ShippingRestrictions\Model\ShippingRestriction::class)->load($id);
            if (!$model->getId() && $id) {
            ...
            try {
                ...
                if ($this->getRequest()->getParam('back')) {
                    return $resultRedirect->setPath('*/*/edit', ['shippingrestriction_id' => $model->getId()]);
                }
                return $resultRedirect->setPath('*/*/');
            } catch (LocalizedException $e) {
                $this->messageManager->addErrorMessage($e->getMessage());
            } catch (\Exception $e) {
                $this->messageManager->addExceptionMessage($e, __('Something went wrong while saving the Shippingrestriction.'));
            }
        
            $this->dataPersistor->set('magendoo_shippingrestrictions_shippingrestriction', $data);
            return $resultRedirect->setPath('*/*/edit', ['shippingrestriction_id' => $this->getRequest()->getParam('shippingrestriction_id')]);
        }
        return $resultRedirect->setPath('*/*/');
    }
}
